...
| `-KeepMedia` | 保留下载来的低清画面（默认**删除**） |
| `-KeepFrames` | 保留抽出的帧（默认**删除**） |

---

## 磁盘占用：跑完几乎不留东西

下载来的音视频只是"为了拿到文字"付出的中间成本。**默认跑完自动清理**，所以你不会在磁盘上留一堆平台内容的副本，`out\` 也不会无限膨胀：

| | 处理前 | 处理后 |
|---|---|---|
| 语音路线（9 分半视频） | 17.6 MB 音轨 | **54 KB**（`transcript.txt` + `metadata.json`） |
| 画面路线（38 帧） | 22.5 MB 画面 + 约 6 MB 帧 | **约 50 KB** |

需要复核或二次处理时，加 `-KeepMedia` / `-KeepFrames` 即可。

**不会在别处留痕迹**：无 yt-dlp 缓存、无系统临时目录残留、无 PyInstaller 解包残留（这正是本项目用嵌入式 Python 而非 `yt-dlp.exe` 的原因）。所有产出只落在你指定的 `-OutDir` 里。
